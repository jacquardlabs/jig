...
def _vocabulary_table_tokens(design_md: str) -> list[str]:
    """Canonical-display tokens from the Vocabulary table's rows that
    fall within `RELEVANT_VOCABULARY_CONCEPTS`."""
    tokens: list[str] = []
    for row in _table_rows(_section(design_md, "Vocabulary")):
        if len(row) < 2:
            continue
        if _plain_text(row[0]) in RELEVANT_VOCABULARY_CONCEPTS:
